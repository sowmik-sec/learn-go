...
func GetUsers() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if startIndexStr := c.Query("startIndex"); startIndexStr != "" {
			if si, err := strconv.Atoi(startIndexStr); err == nil {
				startIndex = si
			}
		}
...
func RefreshToken() gin.HandlerFunc {
	return func(c *gin.Context) {
		log.Println("RefreshToken endpoint hit")
		var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
		defer cancel()
		...
		_, msg := helper.ValidateToken(requestBody.RefreshToken)
		if msg != "" {
		...
		var user models.User
		err := userCollection.FindOne(ctx, bson.M{"refresh_token": requestBody.RefreshToken}).Decode(&user)
		if err != nil {
